...
export default function ProductCards({
  ...
  useEffect(() => {
    ...
    if (initialProducts && initialProducts.length > 0) {
      setProducts(limit ? initialProducts.slice(0, limit) : initialProducts);
      setIsLoading(false);
      return;
    }
    ...
    async function fetchProductsFromBackend() {
      try {
        ...
        const endpoint = category
          ? `${API_BASE_URL}/products/category/${category}${queryLimit}`
          : `${API_BASE_URL}/products${queryLimit}`;
  ...
  const skeletonCount = limit || 10;

  return (
    <section
      data-theme="sage"
      className="w-full bg-[var(--background)] py-8 px-4 sm:px-6 lg:px-8 text-[var(--foreground)]"
    >
